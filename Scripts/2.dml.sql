-- 회원정보
DROP TABLE IF EXISTS web_funding_prj.UserInfo RESTRICT;

-- 회원(판매자)
DROP TABLE IF EXISTS web_funding_prj.UserSeller RESTRICT;

-- 프로젝트
DROP TABLE IF EXISTS web_funding_prj.Project RESTRICT;

-- 프로젝트구매옵션
DROP TABLE IF EXISTS web_funding_prj.PrjOption RESTRICT;

-- 후원정보
DROP TABLE IF EXISTS web_funding_prj.FundingInfo RESTRICT;

-- 관리자
DROP TABLE IF EXISTS web_funding_prj.Admin RESTRICT;

-- 메세지
DROP TABLE IF EXISTS web_funding_prj.Message RESTRICT;

-- 프로젝트게시판
DROP TABLE IF EXISTS web_funding_prj.PrjBoard RESTRICT;

-- Q&A
DROP TABLE IF EXISTS web_funding_prj.QnA RESTRICT;

-- 사이트게시판
DROP TABLE IF EXISTS web_funding_prj.Board RESTRICT;

-- 글분류
DROP TABLE IF EXISTS web_funding_prj.BoardCategory RESTRICT;

-- 주소
DROP TABLE IF EXISTS web_funding_prj.addresses RESTRICT;

-- web_funding_prj
DROP SCHEMA IF EXISTS web_funding_prj;

-- mybatis_study
DROP SCHEMA IF EXISTS mybatis_study;

-- web_funding_prj
CREATE SCHEMA web_funding_prj;

-- mybatis_study
CREATE SCHEMA mybatis_study;

-- 회원정보
CREATE TABLE web_funding_prj.UserInfo (
...
-- 회원정보
ALTER TABLE web_funding_prj.UserInfo
...
-- 회원(판매자)
CREATE TABLE web_funding_prj.UserSeller (
	UserNo      CHAR(10)    NOT NULL COMMENT '회원번호', -- 회원번호
...
-- 회원(판매자)
ALTER TABLE web_funding_prj.UserSeller
...
-- 프로젝트
CREATE TABLE web_funding_prj.Project (
	...
	EndYN      TINYINT     NOT NULL COMMENT '마감여부' -- 마감여부
)
COMMENT '프로젝트';

-- 프로젝트
ALTER TABLE web_funding_prj.Project
...
-- 프로젝트구매옵션
CREATE TABLE web_funding_prj.PrjOption (
...
-- 프로젝트구매옵션
ALTER TABLE web_funding_prj.PrjOption
...
-- 후원정보
CREATE TABLE web_funding_prj.FundingInfo (
	FundingNo CHAR(10) NOT NULL COMMENT '후원번호', -- 후원번호
	UserNo    CHAR(10) NOT NULL COMMENT '회원번호', -- 회원번호
	OptNo     CHAR(10) NOT NULL COMMENT '옵션번호', -- 옵션번호
	AccountNo INT(20)  NOT NULL COMMENT '계좌(카드)번호', -- 계좌(카드)번호
	PayYN     TINYINT  NOT NULL COMMENT '결제여부', -- 결제여부
	EndYN     TINYINT  NOT NULL COMMENT '마감여부' -- 마감여부
)
COMMENT '후원정보';

-- 후원정보
ALTER TABLE web_funding_prj.FundingInfo
...
-- 관리자
CREATE TABLE web_funding_prj.Admin (
...
-- 관리자
ALTER TABLE web_funding_prj.Admin
...
-- 메세지
CREATE TABLE web_funding_prj.Message (
	...
	ReadYN      TINYINT      NOT NULL COMMENT '확인여부' -- 확인여부
)
COMMENT '메세지';

-- 메세지
ALTER TABLE web_funding_prj.Message
...
-- 프로젝트게시판
CREATE TABLE web_funding_prj.PrjBoard (
...
-- 프로젝트게시판
ALTER TABLE web_funding_prj.PrjBoard
...
-- Q&A
CREATE TABLE web_funding_prj.QnA (
...
-- Q&A
ALTER TABLE web_funding_prj.QnA
...
-- 사이트게시판
CREATE TABLE web_funding_prj.Board (
...
-- 사이트게시판
ALTER TABLE web_funding_prj.Board
...
-- 글분류
CREATE TABLE web_funding_prj.BoardCategory (
	CategoryNo   INT(1)      NOT NULL COMMENT '게시글 분류', -- 게시글 분류
...
-- 글분류
ALTER TABLE web_funding_prj.BoardCategory
...
-- 주소
CREATE TABLE web_funding_prj.addresses (
...
-- 주소
ALTER TABLE web_funding_prj.addresses
	ADD CONSTRAINT
		PRIMARY KEY (
			addr_id -- 주소코드
		);
...
ALTER TABLE web_funding_prj.UserSeller
	ADD CONSTRAINT FK_UserInfo_TO_UserSeller -- 회원정보 -> 회원(판매자)
		...
		REFERENCES web_funding_prj.UserInfo ( -- 회원정보
			UserNo -- 회원번호
		);
...
ALTER TABLE web_funding_prj.PrjOption
	ADD CONSTRAINT FK_Project_TO_PrjOption -- 프로젝트 -> 프로젝트구매옵션
		...
		REFERENCES web_funding_prj.Project ( -- 프로젝트
			PrjNo -- 프로젝트번호
		);
...
-- 후원정보
ALTER TABLE web_funding_prj.FundingInfo
	ADD CONSTRAINT FK_PrjOption_TO_FundingInfo -- 프로젝트구매옵션 -> 후원정보
		FOREIGN KEY (
			OptNo -- 옵션번호
		)
		REFERENCES web_funding_prj.PrjOption ( -- 프로젝트구매옵션
			OptNo -- 옵션번호
		);
...
-- 프로젝트게시판
ALTER TABLE web_funding_prj.PrjBoard
	ADD CONSTRAINT FK_UserInfo_TO_PrjBoard -- 회원정보 -> 프로젝트게시판
		FOREIGN KEY (
			UserNo -- 글작성자
		)
		REFERENCES web_funding_prj.UserInfo ( -- 회원정보
...
ALTER TABLE web_funding_prj.QnA
	ADD CONSTRAINT FK_Admin_TO_QnA -- 관리자 -> Q&A
		...
		REFERENCES web_funding_prj.Admin ( -- 관리자
			AdminId -- 관리자아이디
		);
...
ALTER TABLE web_funding_prj.Board
	ADD CONSTRAINT FK_BoardCategory_TO_Board -- 글분류 -> 사이트게시판
		...
		REFERENCES web_funding_prj.BoardCategory ( -- 글분류
			CategoryNo -- 게시글 분류
		);